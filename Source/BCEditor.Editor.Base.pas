...
uses
  ...
  BCEditor.XPlatform, StrUtils, System.UITypes;
...
type
  ...
  protected
    ...
    function DoSearchMatchNotFoundWraparoundDialog(fromBegin: Boolean): Boolean; virtual;
...
uses
  ShellAPI, Imm, Math, Types, Clipbrd, Character, Menus, XClipboard,
...
begin
  ...
  if      Assigned(FLineNumbersCache)
      and (ADisplayLineNumber < Length(FLineNumbersCache))
      and (FLineNumbersCache[ADisplayLineNumber] = ADisplayLineNumber) then
    Result := ADisplayLineNumber
  else
  begin
...
begin
  ...
  try
    ...
    on Exception do
    if LRetryCount = BCEDITOR_CLIPBOARD_MAX_RETRIES then begin
      Break;
    end else begin
      Sleep(LDelayStepMs);
...
begin
  if FMinimap.Align = maRight then
    ARect.Right := ClientRect.Right-ClientRect.Left - FMinimap.GetWidth
  else
...
begin
  ...
  try
    ...
    if LTextCaretPosition.Char <= Length(FLines[LTextCaretPosition.Line]) then
...
begin
  if FMinimap.Align = maRight then
  begin
    ALeft := ClientRect.Right-ClientRect.Left - FMinimap.GetWidth;
    ARight := ClientRect.Right-ClientRect.Left;
  end
  else
  begin
    ALeft := 0;
...
begin
  ...
  begin
    ...
    FBufferBmp.Width := (ClientRect.Right-ClientRect.Left);
    FBufferBmp.Height := (ClientRect.Bottom-ClientRect.Top);
  end;
end;
...
function TBCBaseEditor.DoSearchMatchNotFoundWraparoundDialog(fromBegin: Boolean): Boolean;
var
   fmt : String;
begin
   if fromBegin then
      fmt := SBCEditorSearchMatchNotFoundBegin
   else fmt := SBCEditorSearchMatchNotFoundEnd;
  Result := MessageDialog(Format(fmt, [SLineBreak + SLineBreak]), mtConfirmation, [mbYes, mbNo]) = mrYes;
end;
...
begin
  ...
  LTextLinesRight := (ClientRect.Right-ClientRect.Left);
  if FMinimap.Align = maLeft then
...
begin
  ...
  if FCompletionProposal.Enabled and (not ReadOnly) and not Assigned(FCompletionProposalPopupWindow) then
  begin
    ShortCutToKey(FCompletionProposal.ShortCut, LShortCutKey, LShortCutShift);
    if (AShift = LShortCutShift) and (AKey = LShortCutKey) or
      (AKey <> LShortCutKey) and (cpoAutoInvoke in FCompletionProposal.Options) and Char(AKey).IsLetter() then
    begin
      DoExecuteCompletionProposal;
...
begin
  ...
  if FSearch.Map.Visible then
    if (FSearch.Map.Align = saRight) and (X > (ClientRect.Right-ClientRect.Left) - FSearch.Map.GetWidth) or
...
begin
  ...
  try
    ...
    if FMinimap.Visible then
      ...
      begin
        ...
        begin
          ...
          LLine2 := Min(FLineNumbersCount, LLine1 + (LClipRect.Bottom-LClipRect.Top) div Max(FMinimap.CharHeight - 1, 1));
        end;
        ...
        BitBlt(FMinimapBufferBmp.Canvas.Handle, 0, 0, (DrawRect.Right-DrawRect.Left), (DrawRect.Bottom-DrawRect.Top), Canvas.Handle, DrawRect.Left,
    ...
    { Search map }
    if FSearch.Map.Visible then
      if (FSearch.Map.Align = saRight) and (LClipRect.Right > (ClientRect.Right-ClientRect.Left) - FSearch.Map.GetWidth) or
...
begin
  ...
  XPatBlt(Canvas.Handle, AClipRect, PATCOPY); { fill code folding rect }
  ...
  begin
    ...
    begin
      ...
      XPatBlt(Canvas.Handle, AClipRect, PATCOPY); { active line background }
    end;
      ...
      if Assigned(LCodeFoldingRange) then
        ...
        begin
          ...
          begin
            ...
            begin
              if AMinimap then
                X := (ClientRect.Right-ClientRect.Left) - FMinimap.GetWidth + X
              else
...
var
  ...
  begin
    ...
    begin
      f := nil;
      if CurrentPPI <> FLeftMargin.Font.PixelsPerInch then begin
         f := TFont.Create;
         f.Assign(FLeftMargin.Font);
         f.Height := f.Height * CurrentPPI div FLeftMargin.Font.PixelsPerInch;
         FTextDrawer.SetBaseFont(f);
      end else begin
      ...
      try
        ...
        begin
          ...
          Windows.ExtTextOut(Canvas.Handle, LLineRect.Left + (FLeftMargin.GetWidth - FLeftMargin.LineState.Width - 2) - LTextSize.cx,
  ...
  begin
    ...
    begin
      LPanelRect := Types.Rect(AClipRect.Left, 0, AClipRect.Left + FLeftMargin.Bookmarks.Panel.Width, ClientHeight);
      if FLeftMargin.Colors.BookmarkPanelBackground <> clNone then
      begin
        Canvas.Brush.Color := FLeftMargin.Colors.BookmarkPanelBackground;
        XPatBlt(Canvas.Handle, LPanelRect, PATCOPY); { fill bookmark panel rect }
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            LPanelActiveLineRect := Types.Rect(AClipRect.Left, (i - TopLine) * FLineHeight, AClipRect.Left + FLeftMargin.Bookmarks.Panel.Width,
              (i - TopLine + 1) * FLineHeight);
            Canvas.Brush.Color := FLeftMargin.Colors.ActiveLineBackground;
            XPatBlt(Canvas.Handle, LPanelActiveLineRect, PATCOPY); { fill bookmark panel active line rect}
          end;
        end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          XPatBlt(Canvas.Handle, LLineStateRect, PATCOPY); { fill line state rect }
        end;
      end;
      Canvas.Brush.Color := LOldColor;
    end;
...
begin
  ...
  XPatBlt(Canvas.Handle, AClipRect, PATCOPY); { fill left margin rect }
  PaintLineNumbers;
  PaintBookmarkPanel;
...
begin
  ...
  begin
    ...
    Width := (AClipRect.Right-AClipRect.Left);
...
begin
  ...
  FMinimapShadowBitmap.Height := AClipRect.Bottom-AClipRect.Top;
...
begin
  ...
  {$ENDIF}
    Canvas.Brush.Color := FBackgroundColor;
  XPatBlt(Canvas.Handle, AClipRect, PATCOPY); { fill search map rect }
  { Lines in window }
  LHeight := (ClientRect.Bottom-ClientRect.Top) / Max(Lines.Count, 1);
  AClipRect.Top := Round((TopLine - 1) * LHeight);
  AClipRect.Bottom := Max(Round((TopLine - 1 + VisibleLines) * LHeight), AClipRect.Top + 1);
  Canvas.Brush.Color := FBackgroundColor;
  XPatBlt(Canvas.Handle, AClipRect, PATCOPY); { fill lines in window rect }
  { draw lines }
...
var
  ...
  begin
    ...
    begin
      if FMinimap.Align = maRight then
        Result := (ClientRect.Right-ClientRect.Left) - FMinimap.GetWidth
      else
        Result := 0;
      if FSearch.Map.Align = saRight then
      ...
      begin
        ...
        begin
          ...
          if (soFromEndOfLine in FSelection.Options) and (soToEndOfLine in FSelection.Options) then
            ...
            LTokenRect.Right := X1;
          XPatBlt(Canvas.Handle, LTokenRect, PATCOPY); { fill end of line rect }
          if (soFromEndOfLine in FSelection.Options) and (soToEndOfLine in FSelection.Options) then
          ...
          begin
            ...
            repeat
              ...
              if (LElement <> BCEDITOR_ATTRIBUTE_ELEMENT_COMMENT) and (LElement <> BCEDITOR_ATTRIBUTE_ELEMENT_STRING) then
                Break;
              LOpenTokenEndPos := PosEx(LFoldRange.RegionItem.OpenTokenEnd, AnsiUpperCase(LFromLineText), LOpenTokenEndPos + 1);
...
begin
  ...
  begin
    ...
    SetDrawingColors(False);
    XPatBlt(Canvas.Handle, LTokenRect, PATCOPY);
  end;
    ...
    csVerticalLine, csThinVerticalLine:
      begin
        if LCaretStyle = csVerticalLine then
          LWidth := 2;
        LHeight := FLineHeight;
        LWidth := CurrentPPI * LWidth div 96;
      end;
  end;
  Exclude(FStateFlags, sfCaretVisible);
...
begin
  ...
  if FSearch.Map.Visible and (
    (FSearch.Map.Align = saRight) and (LCursorPoint.X > (ClientRect.Right-ClientRect.Left) - FSearch.Map.GetWidth) or
...
begin
  ...
  begin
    ...
    if (soShowSearchMatchNotFound in FSearch.Options) and DoSearchMatchNotFoundWraparoundDialog(False) or
...
begin
  ...
  begin
    ...
    if (soShowSearchMatchNotFound in FSearch.Options) and DoSearchMatchNotFoundWraparoundDialog(True) or
      (soWrapAround in FSearch.Options) then
    begin
      CaretZero;
      Result := FindNext;
...
begin
   Result := 0;
   if (AStart > 0) and (AStart <= Length(ALine)) then begin
      for i := AStart downto 1 do begin
        if IsWordChar(ALine[i]) and ((i=1) or IsWordBreakChar(ALine[i-1])) then
           Exit(i);
      end;
  end;
end;
...
// CaretEndOfFile
//
procedure TBCBaseEditor.CaretEndOfFile;
var
  LTextCaretPosition: TBCEditorTextPosition;
begin
  LTextCaretPosition.Line := LineNumbersCount-1;
  LTextCaretPosition.Char := Lines.StringLength(LTextCaretPosition.Line)+1;
  TextCaretPosition := LTextCaretPosition;
  SelectionBeginPosition := LTextCaretPosition;
  SelectionEndPosition := LTextCaretPosition;
end;
    ...
    case ACommand of
      ...
      ecDeleteWord, ecDeleteEndOfLine:
        ...
        begin
          LLineText := FLines[LTextCaretPosition.Line];
          LLength := Length(LLineText);
          if ACommand = ecDeleteWord then begin
          ...
          begin
            FLines[LTextCaretPosition.Line] := Copy(LLineText, 1, LTextCaretPosition.Char - 1) + Copy(LLineText, LWordPosition.Char);
            ...
            FUndoList.AddChange(
               ...
               Copy(LLineText, LTextCaretPosition.Char, LWordPosition.Char-LTextCaretPosition.Char), smNormal
               );
          end;
        end;
...
procedure TBCBaseEditor.ExportToHTML(const AFileName: string; const ACharSet: string = ''; AEncoding: SysUtils.TEncoding = nil);
var
...
procedure TBCBaseEditor.ExportToHTML(AStream: TStream; const ACharSet: string = ''; AEncoding: SysUtils.TEncoding = nil);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      LEndLine := LSelectionEndPosition.Line;
      if (LLine <> LEndLine) and (LSelectionEndPosition.Char <= 1) then
         Dec(LEndLine);
    end
    else
    begin
